...
class IndexView(generic.ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["user_menu"] = CustomUser.objects.all()
        return context


def CategoryView(request, cats):
    category_post = NewsStory.objects.filter(category__slug=cats)
    # query the category field defined in the model

    return render(
        request, "news/categories.html", {"cats": cats, "category_post": category_post}
    )
...
class UpdateStoryView(LoginRequiredMixin, UserPassesTestMixin, generic.UpdateView):
    ...
    def get_absolute_url(self):
        return reverse_lazy("news:story", kwargs={"pk": self.pk})
